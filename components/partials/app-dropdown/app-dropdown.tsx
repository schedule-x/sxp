...
export default function AppDropdown({ items, selectedItem, onSelect }: props) {
  ...
  const handleClickOnSelectionItem = (optionValue: string) => {
    setIsOpen(false)
    onSelect && onSelect(optionValue)
    setSelectedOption(items.find((item) => item.value === optionValue))
  }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const navigateUpOrDown = (keyboardEvent: KeyboardEvent | any, optionValue: string) => {
          ...
          {items.map((view) => (
            <li
              ...
              onKeyDown={(keyboardEvent) =>
                navigateUpOrDown(keyboardEvent, view.value)
              }
              onClick={() => handleClickOnSelectionItem(view.value)}
              className={
                'app-dropdown-item' +
